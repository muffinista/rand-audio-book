...
def to_ssml(str):
    output = "<speak version='1.0' xml:lang='en-US' xmlns='http://www.w3.org/2001/10/synthesis' xmlns:mstts='http://www.w3.org/2001/mstts'>" \
        "<voice name='Microsoft Server Speech Text to Speech Voice (en-US, " + voice + ")'>" + str + \
        "</voice></speak>"
...
with open("data/single-digits.txt", encoding="utf-8") as f:
    read_data = f.read()
...
# docker build -t rand-audio -f Dockerfile . && docker run --rm -it -v "$PWD":/usr/src/app -v "$PWD/../rand-audio-samples":/usr/src/app/samples rand-audio ./sample-single-digits.py
